...
public static class LocaleDiscovery
{
    ...
    /// </summary>
    public static IReadOnlyList<MergedTranslation> EntriesForLocale(
        IReadOnlyList<MergedTranslation> entries,
        string locale)
    {
        return entries
            .Where(e => e.InlineTranslations is not null
                     && e.InlineTranslations.ContainsKey(locale))
            .Select(e => e with { SourceText = e.InlineTranslations![locale] })
            .ToList();
    }
}
